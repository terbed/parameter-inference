...
from module.simulation import real_morphology_model_2_spatial
...
p_range = [[50., 200.], [0.00002, 0.0005], [-0.0005, 0.0005]]
p_mean = [100., 0.0001, 0.]     # Fixed prior mean
p_std = [100., 0.0003, 0.0005]      # Fixed prior std
...
def aut_corr_func(x):
    y = A*np.exp(-np.abs(x)/T1)*np.cos(2*np.pi/T2*x)
    if x == 0:
        y += 0.0035
    return y
...
model = partial(real_morphology_model_2_spatial, stim=s)
