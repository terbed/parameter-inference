...
def protocol_comparison(path_list, numfp, inferred_params, out_path, dbs,
    ...
    plt.errorbar(range(len(path_list)), avrg_KL, yerr=std_KL, fmt='none', marker='x')
    plt.grid()
    ...
    for idx, param in enumerate(inferred_params):
        ...
        plt.errorbar(range(len(path_list)), avrg_broad[:, idx], yerr=std_broad[:, idx], label=param + " std", fmt='none', marker='x')
    plt.legend(loc="best")
    plt.grid()
    ...
    for idx, param in enumerate(inferred_params):
        ...
        plt.errorbar(range(len(path_list)), avrg_acc[:, idx], yerr=std_acc[:, idx], label=param + " std", fmt='none', marker='x')
        plt.legend(loc="best")
        plt.grid()
    ...
    for idx, param in enumerate(inferred_params):
        ...
        plt.errorbar(range(len(path_list)), avrg_rdiff[:, idx], yerr=std_rdiff[:, idx], label=param + " std", fmt='none', marker='x')
        plt.legend(loc="best")
        plt.grid()
