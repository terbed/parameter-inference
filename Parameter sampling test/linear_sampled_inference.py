...
def normalDistributionValueGenerator(mean, sigma, x):
    normal_distribution_value = 1/np.sqrt(2*np.pi*sigma**2)*np.exp(-(x-mean)**2/(2*sigma**2))
    return normal_distribution_value


def priorGenerator_2D(xmean, xsigma, x_values, ymean, ysigma, y_values):
    prior = np.zeros( (len(x_values), len(y_values)) )
    for i in range(len(x_values)):
        for j in range(len(y_values)):
            prior[i,j] = normalDistributionValueGenerator(xmean, xsigma, x_values[i]) * normalDistributionValueGenerator(ymean,ysigma, y_values[j])
...
def logLikelihood2D(x_values, y_values, sigma, target_trace):
    ...
    for i in range( len(x_values) ):
        for j in range( len(y_values) ):
            ...
            log_likelihood = - v_summed_square_dev/(2*sigma**2)
...
# Create 2D prior distribution and multiply
prior2d = priorGenerator_2D(cm_mean, cm_sig, cm_values, gpas_mean, gpas_sig, gpas_values)
likelihood2d = np.multiply(likelihood2d, prior2d)
...
x, y = np.meshgrid(gpas_values, cm_values)
ax = fig.gca(projection='3d')
surf = ax.plot_surface(x, y, prior2d, rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    
ax.zaxis.set_major_locator(LinearLocator(10))
ax.zaxis.set_major_formatter(FormatStrFormatter('%.02f'))
fig.colorbar(surf, shrink=0.5, aspect=5)

# Marginalize g_pas out
...
np.savetxt('data2/posterior_' + ns + 'l' + '.txt', np.c_[cm_values, posterior], delimiter = ' ')
