...
for idx, name in enumerate(folderNames):
    ...
    f_soma, Pxx_den_soma = signal.welch(data[:, 2], Fs, window='hamming', nperseg=100000, scaling='spectrum')
    f_dend, Pxx_den_dend = signal.welch(data[:, 1], Fs, window='hamming', nperseg=100000, scaling='spectrum')

    # plt.figure(figsize=(12, 6))
    axarr[idx, 0].semilogy(f_soma[0:25], np.sqrt(Pxx_den_soma[0:25]), 'b', label='soma')
    axarr[idx, 0].semilogy(f_dend[0:25], np.sqrt(Pxx_den_dend[0:25]), '--g', label='dend')
    ...
    # plt.figure(figsize=(12, 6))
    axarr[idx, 1].semilogy(f_soma[0:25], np.sqrt(Pxx_den_soma[0:25]), 'b', label='soma')
    axarr[idx, 1].semilogy(f_dend[0:25], np.sqrt(Pxx_den_dend[0:25]), '--g', label='dend')
...
f.show()
f.savefig('/home/terbe/parameter-inference/Noise/Data/AnalyseNoise/10Hz2.png')
